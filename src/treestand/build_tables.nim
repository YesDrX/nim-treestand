...
proc buildTables*(syntaxGrammar: SyntaxGrammar, lexicalGrammar: LexicalGrammar): Tables =
  ...
  var parseTable = buildParseTable(syntaxGrammar, lexicalGrammar)

  # 2. Build NFA for lexical grammar
  var mutableLexical = lexicalGrammar
  buildLexicalNfa(mutableLexical)
  
  # 3. Conflict Detection Setup
  # We need to compute FIRST and LAST sets to determine "following tokens"
...
proc buildParseTable*(grammar: SyntaxGrammar, lexicalGrammar: LexicalGrammar): BuildParseTable =
  ...
  # Build LR states - use optimized LALR or traditional Canonical LR(1)
  when USE_LALR_OPTIMIZED:
        ...
        if stateId < transitions.len and nextSym in transitions[stateId]:
          ...
          if nextSym.kind != stNonTerminal:
            let newAction = BuildParseAction(
              kind: bpakShift,
              participants: @[GrammarSymbol(kind: stNonTerminal, index: item.variableIndex.uint16)],
