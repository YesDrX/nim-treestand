...
proc generateParseTableCode*(parseTable: BuildParseTable, lexicalGrammar: LexicalGrammar): string =
  ...
  for i, entry in parseTable.entries:
    ...
    for (sym, action) in entry.actionMap:
      # Symbol
      let symCode = case sym.kind
        of stNonTerminal: &"parser_types.nt({sym.index})"
        of stTerminal: &"parser_types.t({sym.index + 1})"
        of stExternal: &"parser_types.t({externalTokenBase + sym.index.int})"
        else: "parser_types.t(0)" 
...
proc generateLexerRuntime*(lexTable: BuildLexTable, lexicalGrammar: LexicalGrammar, externalTokens: seq[string], extras: seq[GrammarSymbol]): string =
  ...
  lines.add("    advance(lexer)") # Use advance
  ...
  if extras.len > 0:
    ...
    lines.add("    ")
    lines.add("    # Skip whitespace again")
    lines.add("    while lexer.pos < lexer.input.len and lexer.input[lexer.pos] in {'\\t', ' ', '\\r', '\\n'}:")
...
proc generateParser*(grammarName: string, grammarPath: string, 
  ...
  lines.add(generateParseTableCode(parseTable, lexicalGrammar))
  lines.add("")
