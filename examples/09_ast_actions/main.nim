...
when isMainModule:
  echo "=== AST Construction with Embedded Actions ==="
  echo ""
  ...
  block:
    echo "Example 2: 1 + 2 * 3"
    var builder = AstBuilder()
    
    if matchMath("1 + 2 * 3", builder):
      echo "  Root AST: ", builder.rootAst
      echo "  Expected: ((1 + 2) * 3)  # Left-associative"
      echo ""
    else:
      echo "  Parse failed!"
  ...
  block:
    echo "Example 4: 1 + 2 * 3 - 4 / 5"
    var builder = AstBuilder()
    
    if matchMath("1 + 2 * 3 - 4 / 5", builder):
      echo "  Root AST: ", builder.rootAst
      echo ""
    else:
      echo "  Parse failed!"
